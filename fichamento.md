...
## Nota de atualização (Marco 2)

A escolha da modalidade T1wCE em vez de T1w para a extração de descritores
(ver `formulacao_problema.md`) foi feita por **experimentação/ablação do
próprio grupo**, não por citação direta de um dos 10 artigos acima — nenhum
deles compara diretamente T1w vs. T1wCE isoladamente. Essa é uma decisão
metodológica amparada pelo TP1 (§4.1, que aceita justificativa por "evidência
empírica do próprio grupo") e deve ser apresentada no artigo como tal, sem
atribuí-la à literatura fichada aqui.
