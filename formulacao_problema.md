...
- **Modalidade priorizada para extração de features: T1wCE (não T1w).**
  Decisão tomada no Marco 2 com base em **evidência empírica do próprio
  grupo** (grade de 8 combinações modalidade × distância GLCM × níveis de
  cinza, avaliadas com a partição por paciente já congelada) — critério
  explicitamente permitido pelo TP1 quando não há literatura direta (§4.1:
  "ancorado em literatura ou em evidência empírica do próprio grupo
  (ablação)"). T1wCE superou T1w em AUC nas 4 combinações testadas (melhor
  configuração: AUC 0,751 ± 0,114 contra 0,584 ± 0,219 do T1w). Prova de
  que a etapa de experimentação (exigida pelo professor nas fases 1 e 2)
  mudou uma decisão de fato, em vez de apenas confirmar a escolha inicial.
- **Parâmetros do GLCM confirmados por experimentação:** distância = 1
  pixel, 32 níveis de cinza — mesma grade de experimentos acima, escolhidos
  pelo equilíbrio entre AUC alto e menor desvio-padrão entre dobras dentro
  das configurações de T1wCE.
...
## Status do Marco 2 (concluído) / Pendências para o Marco 3

- [x] Implementar o pipeline de pré-processamento (normalização por
      percentil, recorte de região não-nula, redimensionamento)
- [x] Partição por paciente com `StratifiedGroupKFold`, congelada
- [x] Baseline trivial rodando com métrica reportada (acurácia 0,40 —
      efeito esperado de amostra pequena e quase balanceada, documentado)
- [x] Primeira família de descritores extraída (GLCM/Haralick) + um
      classificador treinado (regressão logística; AUC 0,751 ± 0,114,
      acima do baseline trivial)
- [x] Experimentação de parâmetros (modalidade × distância × níveis de
      cinza) — decisão de usar T1wCE em vez de T1w tomada com evidência
      empírica
...
- [ ] Definir a estratégia de agregação de cortes por paciente (2D com
      pooling / 2.5D / 3D)
- [ ] Extrair as demais famílias de descritores (mínimo 3 no total)
